...
pub fn _search_directory(directory: String, file_ext: String, pattern: String) -> Result<Vec<TodoMatch>, Error> {
...
#[defun]
fn search_directory(env: &Env, directory: String, file_ext: String, pattern: String) -> emacs::Result<Value> {
    let found = _search_directory(directory, file_ext, pattern)?;

    let elisp_results = found
        .into_iter()
        .map(|todo| {
            env.vector((todo.file_path, todo.line_number, todo.line))
        })
...
mod tests {
    ...
    fn files_to_search_returns_the_expected_files() -> Result<(), String> {
        let dir = tempdir().map_err(|e| format!("Unable to create a temporary directory: {:?}", e))?;
        let file_path = dir.path().join("my-temporary-file.md");
        let mut file1 = File::create(&file_path).map_err(|e| format!("Unable to create a temp file: {:?}", e))?;
        writeln!(file1, "Test data.").map_err(|e| format!("Unable to write to temp file: {:?}", e))?;

        let expected = vec![file_path.into_os_string()];
        let actual = files_to_search(dir.into_path().into_os_string().to_string_lossy().to_string(), ".md".to_string());
    ...
    fn search_directory_returns_the_expected_matches() -> Result<(), String> {
        ...
        let mut file1 = File::create(&file_path).map_err(|e| format!("Unable to create a temp file: {:?}", e))?;
        writeln!(file1, "- [ ] do your taxes").map_err(|e| format!("Unable to write to temp file: {:?}", e))?;

        let pattern = r"^- ?\[[Xx ]\] ".to_string();
        let expected: Vec<TodoMatch> = vec![TodoMatch::new(file_path.into_os_string(), 1, "- [ ] do your taxes")];
        let actual = _search_directory(
            dir.into_path().into_os_string().to_string_lossy().to_string(),
            ".md".to_string(),
            pattern).unwrap();
